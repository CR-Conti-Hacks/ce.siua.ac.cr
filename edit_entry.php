...
$entry_fields = array( 'name', 'description', 'type', 'start_date', 'end_date', 'areas','rooms',  'confirmation_status', 'privacy_status');
...
function create_field_entry_name($disabled=FALSE)
{
  ...
  if($name==''){
    //$name = 'SIUA_TEC_temporal_'.date('Y');
  }
...
function create_field_entry_description($disabled=FALSE)
{
  ...
  else
  {
    ...
    /*************************************************************************************************************************/
    // ORIGINAL ------>generate_textarea($params);
    /*************************************************************************************************************************/
    generate_input($params);
...
function create_field_entry_rooms($disabled=FALSE)
{
  ...
  /*************************************************************************************************************************/
  //OCULTO ---------------------------------------------------------------------------------->
  //echo "<div style='display:none' id=\"div_rooms\">\n";
  /*************************************************************************************************************************/
...
/********************************************************     Crea Campos Personalizados  *********************************************************************/
/**************************************************************************************************************************************************************/
    //Si es un usuario basico Fecha de reserva
    /*if($field['name']=='fecha_reserva'){
      //si es una nueva reserva agregue la fecha
      if($field['value']==''){
        $hoy = date("d/m/Y");
        $params['value'] = $hoy;  
      }
      if(authGetUserLevel($user) <=1){
        ...
        $params['type'] = 'hidden';
        $params['style'] = 'display:none';
      }elseif(authGetUserLevel($user) >1){
        $params['disabled'] = TRUE;
        $params['type'] = 'text';
      }
    }
    ...
    if(
       ...
       ($field['name']=='Kilometraje_fin') ||
       ($field['name']=='Kilometraje_diferencia'))
       && ($params['value']=='')      
       ){
        $params['value']=0;
    }
    ...
    if((($field['name']=='Horas_servicio_diferencia_manana') || ($field['name']=='Horas_servicio_diferencia_tarde'))&&($params['value']=='')){
      $params['readonly']='readonly';
      $params['value'] = '00:00';
    }
    
    if(($field['name']=='Horas_servicio_inicial_manana') || ($field['name']=='Horas_servicio_fin_manana')){
      $params['onchange'] = "restarHoras('f_Horas_servicio_inicial_manana','f_Horas_servicio_fin_manana','f_Horas_servicio_diferencia_manana')";
    }
    
    if(($field['name']=='Horas_servicio_inicial_tarde') || ($field['name']=='Horas_servicio_fin_tarde')){
      $params['onchange'] = "restarHoras('f_Horas_servicio_inicial_tarde','f_Horas_servicio_fin_tarde','f_Horas_servicio_diferencia_tarde')";
    }
    
    if(($field['name']=='Kilometraje_inicial') || ($field['name']=='Kilometraje_fin')){
      $params['onchange'] = "restarKMS('f_Kilometraje_inicial','f_Kilometraje_fin','f_Kilometraje_diferencia')";
    }
   
    if($field['name']=='Kilometraje_diferencia'){
      $params['onchange'] = "CalculaCostoViaje('f_Kilometraje_diferencia','f_tipo_costos','f_costo_viaje')";
      $params['readonly']='readonly';
    }
    
    if($field['name']=='tipo_costos'){
      $params['onchange'] = "CalculaCostoViaje('f_Kilometraje_diferencia','f_tipo_costos','f_costo_viaje')";
      
    }
    
     if($field['name']=='usuarios_viaje'){
      $params['type'] = "hidden";
      $params['style'] = "display:none";
    }
...
/**************************************************************************************************************************************************************/
/********************************************************   FIN Crea Campos Personalizados  *******************************************************************/
/**************************************************************************************************************************************************************/
...
/*function Crear_Miembros()
{
  ...
  //obtener la cantidad de campos de la base de datos
    $sql = "SELECT capacity FROM mrbs_entry
          INNER JOIN mrbs_room ON (mrbs_entry.room_id = mrbs_room.id)
          WHERE mrbs_entry.id=".$_GET['id'];
  ...
  //style='display:none;
  echo "<div 'id=\"div_miembros\">\n";
  
  echo '<table class="tabla_miembros">';
  echo '<tr class="titulo_miembros">';
    echo '<td colspan="4">'.get_vocab('miembros_titulo_0').':'.'</td>';
  echo '</tr>';
  echo '<tr class="titulo_miembros">';
    echo '<td>'.get_vocab('miembros_titulo_1').':'.'</td>';
    echo '<td>'.get_vocab('miembros_titulo_2').':'.'</td>';
    echo '<td>'.get_vocab('miembros_titulo_3').':'.'</td>';
    echo '<td>'.get_vocab('miembros_titulo_4').':'.'</td>';
  echo '</tr>';
  ...
  for($i=1; $i <= $cantidad; $i++){
    echo '<tr>';
      ...
      echo '</td>';
      echo '<td>';
            echo '<input type="text" id="cedula_'.$i.'" name="cedula_'.$i.'" value="" maxlength="16" onblur="obtieneUsuarios('.$cantidad.')"  />';
      echo '</td>';
      echo '<td>';
            echo '<input type="text" id="nombre_'.$i.'" name="nombre_'.$i.'" value="" maxlength="60" onblur="obtieneUsuarios('.$cantidad.')" />';
      echo '</td>';
      echo '<td>';
            echo '<select id="universidad_'.$i.'" onchange="obtieneUsuarios('.$cantidad.')">
                  <option value="UCR">UCR</option>
                  <option value="TEC">TEC</option>
                  <option value="UNA">UNA</option>
                  <option value="UNED">UNED</option>
                  <option value="CONARE">CONARE</option>
...
/**************************************************************************************************************************************************************/
/*******************************************************   FIN obtener los datos de la reserva  ***************************************************************/
/**************************************************************************************************************************************************************/
...
foreach ($edit_entry_field_order as $key)
{
  ...
  default:
    ...
    //echo $key.'<br />';
   /* if($key=='conductor' && authGetUserLevel($user) <=1){
      echo '<input type="hidden" name="f_conductor" id="f_conductor" value="N/A" />';
    }elseif($key=='centro_costo' && authGetUserLevel($user) <=1){
      echo '<input type="hidden" name="f_centro_costo" id="f_centro_costo" value="N/A" />';
    }elseif($key=='Kilometraje_inicial' && authGetUserLevel($user) <=1){
      echo '<input type="hidden" name="f_Kilometraje_inicial" id="f_Kilometraje_inicial" value="0" />';
    }elseif($key=='Kilometraje_fin' && authGetUserLevel($user) <=1){
      echo '<input type="hidden" name="f_Kilometraje_fin" id="f_Kilometraje_fin" value="0" />';
    }elseif($key=='Kilometraje_diferencia' && authGetUserLevel($user) <=1){
      echo '<input type="hidden" name="f_Kilometraje_diferencia" id="f_Kilometraje_diferencia" value="0" />';
    }elseif($key=='Horas_servicio_inicial_manana' && authGetUserLevel($user) <=1){
      echo '<input type="hidden" name="f_Horas_servicio_inicial_manana" id="f_Horas_servicio_inicial_manana" value="00:00" />';
    }elseif($key=='Horas_servicio_fin_manana' && authGetUserLevel($user) <=1){
      echo '<input type="hidden" name="f_Horas_servicio_fin_manana" id="f_Horas_servicio_fin_manana" value="00:00" />';
    }elseif($key=='Horas_servicio_diferencia_manana' && authGetUserLevel($user) <=1){
      echo '<input type="hidden" name="f_Horas_servicio_diferencia_manana" id="f_Horas_servicio_diferencia_manana" value="0" />';
    }elseif($key=='Horas_servicio_inicial_tarde' && authGetUserLevel($user) <=1){
      echo '<input type="hidden" name="f_Horas_servicio_inicial_tarde" id="f_Horas_servicio_inicial_tarde" value="00:00" />';
    }elseif($key=='Horas_servicio_fin_tarde' && authGetUserLevel($user) <=1){
      echo '<input type="hidden" name="f_Horas_servicio_fin_tarde" id="f_Horas_servicio_fin_tarde" value="00:00" />';
    }elseif($key=='Horas_servicio_diferencia_tarde' && authGetUserLevel($user) <=1){
      echo '<input type="hidden" name="f_Horas_servicio_diferencia_tarde" id="f_Horas_servicio_diferencia_tarde" value="0" />';
    }elseif($key=='tipo_costos' && authGetUserLevel($user) <=1){
      echo '<input type="hidden" name="f_tipo_costos" id="f_tipo_costos" value="0" />';
    }elseif($key=='costo_viaje' && authGetUserLevel($user) <=1){
      echo '<input type="hidden" name="f_costo_viaje" id="f_costo_viaje" value="0" />';
    }else{
...
if (($edit_type == "series") && $repeats_allowed)
{
  ...
  /**************************************************************************************************************************************************************/
  //echo "<fieldset style='display:none' id=\"rep_info\">\n";
...
<script>
/*function restarHoras(hora_inicio, hora_fin,resultado) {
  ...
  resultado.value = horas+"."+minutos;

}


function restarKMS(KMS_inicio, KMS_fin,KMS_resultado) {
  
  inicio = document.getElementById(KMS_inicio).value;
  fin = document.getElementById(KMS_fin).value;
  resultado = document.getElementById(KMS_resultado);
  
  resultado.value = parseInt(fin) - parseInt(inicio);
  CalculaCostoViaje('f_Kilometraje_diferencia','f_tipo_costos','f_costo_viaje');
}


function CalculaCostoViaje(diferencia,tipo_costo,costo_viaje){
  diferencia = document.getElementById(diferencia).value;
  tipo_costo = document.getElementById(tipo_costo);
  tipo_costo = tipo_costo.options[tipo_costo.selectedIndex].value;
  document.getElementById(costo_viaje).value = diferencia*tipo_costo;
  
}

function obtieneUsuarios(cantidad) {
  var cedula= '';
  var nombre ='';
  var universidad = '';
  var usuario ='';
  ...
  for(var i =1; i <= cantidad; i++){
    cedula = document.getElementById('cedula_'+i).value;
    nombre = document.getElementById('nombre_'+i).value;
    universidad = document.getElementById('universidad_'+i).value;
    usuario += cedula + ',' + nombre + ',' + universidad + ';';
  }
  
  document.getElementById('f_usuarios_viaje').value = usuario;
}


function cargaUsuarios() {
  var usuarios = document.getElementById('f_usuarios_viaje').value;

  if (usuarios!='') {
    array_usuarios = usuarios.split(';');
    for (var i = 0; i<array_usuarios.length; i++) {
      
      array_usuarios2 = array_usuarios[i].split(',');
      if ((array_usuarios2[0]!='') && (array_usuarios2[1]!='')){
        fila = i +1;
        document.getElementById('cedula_'+ fila).value = array_usuarios2[0];
        document.getElementById('nombre_'+fila).value = array_usuarios2[1];
        
        uni = document.getElementById('universidad_'+fila);
        for (var a = 0; a < uni.options.length; a++) {
          if (uni.options[a].value== array_usuarios2[2]) {
              uni.options[a].selected = true;
              break;
          }
        }  
      }
